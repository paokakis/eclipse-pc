...
static const int32_t VELOCITY_ITER = 1;
static const int32_t POSITION_ITER = 1;
...
b2Body* PhysicsManager::loadBody(glm::vec3& pLocation,
	uint16 pCategory, uint16 pMask, int32_t pSizeX, int32_t pSizeY,
	float pRestitution, int group, float pDensity)
{
	...
	mFixtureDef.filter.groupIndex = group;
...
b2Body* PhysicsManager::loadBulletBody(glm::vec3& pLocation, uint16 pCategory, uint16 pMask, int32_t pSizeX, int32_t pSizeY, float pRestitution, int group, float pDensity)
{
	PhysicsCollision* userData = new PhysicsCollision();
	b2BodyDef mBodyDef;
	b2Body* mBodyObj;
	...
	mBodyDef.userData = userData;
	mBodyDef.awake = true;
	mBodyDef.fixedRotation = false;
	mBodyDef.bullet = true;
	mShapeDef.m_p = b2Vec2(0, 0);
	int32_t diameter = (pSizeX + pSizeY) / 2;
	mShapeDef.m_radius = diameter / (2.5f * PHYSICS_SCALE);
	mFixtureDef.shape = &mShapeDef;
	mFixtureDef.density = pDensity;
	mFixtureDef.friction = 0.0f;
	mFixtureDef.restitution = pRestitution;
...
b2MouseJoint* PhysicsManager::loadTarget(b2Body* pBody)
{
	...
	mouseJointDef.maxForce = 100.0f * pBody->GetMass();
	mouseJointDef.dampingRatio = 1.f;
	mouseJointDef.frequencyHz = 5.f;
