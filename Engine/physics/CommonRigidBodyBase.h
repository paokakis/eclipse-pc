...
struct CommonRigidBodyBase
{
	...
	{
		GraphicsManager* renderer = GraphicsManager::getInstance();
		...
		camPos = renderer->getCameraPos();
		camTarget = renderer->getLookAtPos();
		...
		btVector3 camTargetB3;
		camTargetB3.x = camPos.x;
		camTargetB3.y = camPos.y;
		camTargetB3.z = camPos.z;

		btVector3 rayForward = (camTargetB3 - camPosB3);
		rayForward.normalize();
		float farPlane = renderer->getDepth();
		rayForward *= farPlane;

		btVector3 rightOffset;
		btVector3 cameraUp = btVector3(renderer->getHeadPos().x, renderer->getHeadPos().y, renderer->getHeadPos().z);
		...
		btScalar aspect;
		float width = float(renderer->getRenderWidth());
		float height = float(renderer->getRenderHeight());
