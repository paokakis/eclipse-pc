...
Sprite::Sprite(GraphicsManager& pGraphicsManager, const std::string pTextureResource, int32_t pHeight, int32_t pWidth, glm::vec3& pLoc) :
	location(pLoc),
	...
	mAnimSpeed(0), mAnimFrame(0), mAnimLoop(false), active(false)
{}
...
{
	TextureProperties* textureProperties = pGraphicsManager.loadTextureDiffuse(mTextureResource);
...
void Sprite::draw(Vertex pVertices[4], float pTimeStep)
{
	if (active)
	{
		...
		GLfloat posZ1 = location.z;
		GLfloat posZ2 = location.z;
