...
void main() {
	...
	if (diff > 0.01)
	{
		finalColor = mix(tex1Color, tex3Color, tex3Color.r);
	}
	else
	{
		finalColor = mix(tex2Color, tex3Color, tex3Color.r);
	}
	
	outColor = finalColor * vec4((ambient + diffuse + specular), 1.0);	
	gl_FragColor = outColor;
	
})glsl";

PlanetEarth::PlanetEarth(GraphicsManager& graphMan, TimeManager& timeMan, glm::vec3& pos, glm::vec3& scale,
...
{
	Log::info("Planet Earth constructor");

	mTexturePaths.push_back(EARTH_TEXTURE);
	mTexturePaths.push_back(EARTH_NIGHT);
	mTexturePaths.push_back(EARTH_CLOUDS);
...
{
	Log::info("Planet Earth destructor");
}

status PlanetEarth::load()
{
...
ERROR:
	Log::error("Planet Earth Error Loading textures");
